...
def retrieveWorkoutInfo(workoutUrl):
    ...
    try: bsExercises = workoutBS.select_one(".pull-none").select_one("img[src]")["src"]
    except: bsExercises = ""

    try:bsMuscles = workoutBS.select_one(".infobox-map").select_one("img[src]")["src"]
    except: bsMuscles = ""

    try: bsFocus = workoutBS.select_one(".infobox-works").select_one("img[src]")["src"]
    except: bsFocus = ""
    
    try: bsType = workoutBS.select_one(".infobox-focus").select_one("img[src]")["src"]
    except: bsType = ""

    try: bsDifficulty = workoutBS.select_one(".infobox-difficulty").select_one("img[src]")["src"]
    except: bsDifficulty = ""

    try: bsDescription = workoutBS.select_one(".infotext").contents
    except: bsDescription = ""

    try: bsExtraCredit = workoutBS.select_one(".infoec").contents
    except: bsExtraCredit = ""
    ...
    workoutDifficulty = bsDifficulty[bsDifficulty.find("-")+1:bsDifficulty.find(".")]
